...
public:
	...
	vector<District> getDistrictsFromSolution(vector<set<int>> district);
	void readBlockssJSON();

	void completeRandomDistrict(set<set<int>> &sampledDistricts, set<int> included, set<int> reachable, int nbRemaining);
	void generateTSPinstance(vector<Point> &points, int scenarioNumber);

	
	Params(string Instance_evalution);
};
